...
!!@brief In the current code we parallelized the loop inside the following subroutines:
!!@brief init (the loop for the Gauss-Legendre quadrature), total_mass_energy
!!@{
...
module module_physics
  ...
  subroutine init(etime,output_counter,dt)
    ...
    !$acc enter data copyin(i, k, kk, ii, hs, nx, nqpoints, qpoints, i_beg, k_beg, dx, dz, hr, ht, r, u, w, t, x, z, oldstat, qweights)
    copyout(oldstat, x, z)
    {
    !$acc parallel do collapse(2) present(kk, ii, x, z, r, u, w, t, hr, ht)
  ...
  subroutine total_mass_energy(mass,te)
    ...
    te = 0.0_wp
    call system_clock(tstart)
    !!$acc enter data copyin(i, k, oldstat, ref, c0, th, cdocv, th, p0, p, rdocp, cv, t, mass, dx, dz, te, ke )
    !copyout(r, u, w, ke, ie, mass, te, t, p, th)
    !{
